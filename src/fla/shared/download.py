...
# Environment variable to control cache directory path, ~/.cache/fla will be used by default.
# OPENPI_DATA_HOME is still supported for backward compatibility.
_FLA_DATA_HOME = "FLA_DATA_HOME"
_OPENPI_DATA_HOME = "OPENPI_DATA_HOME"
DEFAULT_CACHE_DIR = "~/.cache/fla"

logger = logging.getLogger(__name__)


def get_cache_dir() -> pathlib.Path:
    cache_dir = pathlib.Path(os.getenv(_FLA_DATA_HOME, os.getenv(_OPENPI_DATA_HOME, DEFAULT_CACHE_DIR))).expanduser().resolve()
...
def _get_mtime(year: int, month: int, day: int) -> float:
    ...
    # Use datetime.timezone.utc for Python 3.10 compatibility (datetime.UTC is 3.11+)
