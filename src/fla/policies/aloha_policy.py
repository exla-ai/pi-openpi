...
def _decode_aloha(data: dict, *, adapt_to_pi: bool = False) -> dict:
    ...
    def convert_image(img):
        ...
        # Accept both CHW and HWC inputs. Default to HWC if the channel dimension is last.
        # Some datasets (e.g., LeRobot) store images as HWC, while Aloha logs CHW.
        if img.shape[0] in (1, 3) and img.shape[-1] not in (1, 3):
            return einops.rearrange(img, "c h w -> h w c")
        if img.shape[-1] in (1, 3):
            return img
        # Fallback: treat small leading dimension as channel-first.
        if img.shape[0] <= 4:
            return einops.rearrange(img, "c h w -> h w c")
        return img
