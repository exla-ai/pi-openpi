...
from fla.training.droid_rlds_dataset import DroidRldsDataset
import fla.transforms as _transforms

T_co = TypeVar("T_co", covariant=True)


def _patch_lerobot_stack() -> None:
    """Patch LeRobot's torch.stack usage to accept dataset columns.

    Some LeRobot versions call torch.stack on datasets Columns, which are not lists of tensors.
    This patch converts iterables of scalars into tensors before stacking.
    """
    if getattr(lerobot_dataset, "_fla_stack_patch", False):
        return

    _orig_stack = lerobot_dataset.torch.stack

    def _safe_stack(tensors, *args, **kwargs):  # type: ignore[override]
        if not isinstance(tensors, (list, tuple)):
            try:
                tensors = list(tensors)
            except TypeError:
                tensors = [tensors]
        if tensors and not isinstance(tensors[0], lerobot_dataset.torch.Tensor):
            return lerobot_dataset.torch.tensor(tensors)
        return _orig_stack(tensors, *args, **kwargs)

    lerobot_dataset.torch.stack = _safe_stack
    lerobot_dataset._fla_stack_patch = True
...
class RLDSDataLoader:
    """Shallow wrapper around the DROID data loader to make it compatible with fla.
